...
class StringService implements GrpcService {
  ...
  private void upper(StringMessage request, StreamObserver<StringMessage> observer) {
    ...
    // .build());
    String requestText = messageText(request);
    complete(observer, newMessage(requestText.toUpperCase(Locale.ROOT)));
  }
  ...
  private void lower(StringMessage request, StreamObserver<StringMessage> observer) {
    ...
    // .build());
    String requestText = messageText(request);
    complete(observer, newMessage(requestText.toLowerCase(Locale.ROOT)));
  }
  ...
  private void split(StringMessage request, StreamObserver<StringMessage> observer) {
    ...
    String[] parts = messageText(request).split(" ");
    stream(observer, Stream.of(parts).map(StringService::newMessage));
  }
  ...
  private StreamObserver<StringMessage> join(StreamObserver<StringMessage> observer) {
    return CollectingObserver.create(
        ...
        StringService::messageText,
        StringService::newMessage);
  }
  ...
  static String messageText(StringMessage msg) {
    return msg.getText().getValue().toStringUtf8();
  }

  static StringMessage newMessage(String text) {
    return StringMessage.newBuilder()
        .setText(Any.newBuilder()
