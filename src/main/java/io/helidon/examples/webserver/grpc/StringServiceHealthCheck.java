...
import io.helidon.examples.webserver.grpc.StringServiceGrpc.StringServiceBlockingStub;
...
class StringServiceHealthCheck implements HealthCheck {
  ...
  private void checkReadiness() {
    try {
      ...
      StringServiceBlockingStub service = StringServiceGrpc.newBlockingStub(grpcClient.channel());
      StringMessage res = service.upper(StringService.newMessage("hello"));
      readiness = StringService.messageText(res).equals("HELLO");
    } catch (Exception e) {
      readiness = false;
    } finally {
      latch.countDown();
